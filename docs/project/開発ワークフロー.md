...
ただし、何を依頼する場合でも、Codex は原則として `.steering/` を作成して作業ログを残します。
...
```
共通ルール:
  └── ユーザーが AI に作業指示を出したら、原則として毎回 `.steering/[日付]-[作業名]/` を作成する

フェーズ0: アイデア壁打ち（必要なら最初に実施）
  └── brainstorm
        ├── ステアリング作成（steering スキル）
        ├── 作りたいものを短く整理
        ├── 誰向けか、最初に必要な機能、制約を整理
        └── docs/ideas/initial-requirements.md の草案を作成

フェーズ1: プロジェクト立ち上げ（初回のみ）
  └── setup-repo / setup-project
        ├── ステアリング作成（steering スキル）
...
フェーズ2: 機能追加（繰り返す）
  └── add-feature [機能名]
        ...
        ├── 実装検証（validate-implementation スキル）
        ├── テスト・lint・型チェック
        └── 振り返り記録（steering スキル）
...
要件がまだ曖昧なときは、最初に `brainstorm` で壁打ちします。
目的は設計を完成させることではなく、`docs/ideas/initial-requirements.md` に書ける初期要件まで整理することです。
このフェーズでも `.steering/` を作成し、今回の相談内容と整理結果を残します。
...
`brainstorm` の最終出力は、`docs/ideas/initial-requirements.md` にそのまま転記できる Markdown 草案を前提にします。
加えて、`.steering/` に今回の相談内容、整理した論点、未確定事項を残します。

#### Codex にはこう依頼する

```text
brainstorm でアプリ案を壁打ちして。
```

```text
steering と brainstorm でアプリ案を壁打ちして。
```

```text
brainstorm でポモドーロアプリの initial-requirements を固めて。
```
...
### Skill: `setup-repo` / `setup-project`
...
**依頼例**:
```
add-feature でユーザープロフィール編集を追加して。
add-feature で記事一覧ページを実装して。
add-feature でコメント機能を進めて。
```

#### テスト駆動の基本

`add-feature` では、できる限り次の順で進めます。
...
詳細な流れは `docs/project/機能追加の流れ.md` の TDD アプローチを参照します。
...
Step 6: 実装検証 ← validate-implementation スキル
...
`.steering/` は Codex が AI への各作業指示ごとに作る「作業ログ」です。
...
実装、壁打ち、レビュー、文書化のいずれでも、原則として `.steering/` を残します。
...
```text
steering と add-feature でユーザープロフィール編集を追加して。
```
...
```text
add-feature で記事一覧ページを実装して。大きなリファクタはしないで。
```

```text
add-feature でコメント機能を進めて。必要なら docs も更新して。
```
...
### Skill: `review-doc [ファイルパス]`

ドキュメントの品質を確認したいときに実行します。

**依頼例**:
```
review-doc で docs/continuous/プロダクト要求定義.md を見て。
review-doc で docs/continuous/機能設計.md をレビューして。
```

#### 評価される観点（review-doc スキル）
...
```text
steering と review-doc で docs/continuous/プロダクト要求定義.md を見て。
```
...
```text
review-doc で docs/continuous/機能設計.md をレビューして。修正案までほしい。
```
...
| 入口 | いつ使う |
|---|---|
| `brainstorm` | `initial-requirements` を作る前に壁打ちしたいとき |
| `setup-repo` / `setup-project` | プロジェクト開始時に1回 |
| `add-feature [機能名]` | 機能を追加するたびに |
| `review-doc [パス]` | ドキュメントをレビューしたいとき |

### 補助スキル

| スキル | 役割 | 使用タイミング |
|---|---|---|
| `brainstorm` | 初期要件の壁打ちと整理 | 立ち上げ前 |
| `prd-writing` | PRD 作成 | `setup-repo` / `setup-project` の序盤 |
...
| `validate-implementation` | 実装コードの品質検証 | `add-feature` の検証時 |
| `review-doc` | ドキュメントの品質レビュー | レビュー時 |

---

## Git との連携

機能追加の流れは `docs/project/Gitブランチ運用ルール.md` の「新機能の開発」パターンに従います。
...
`add-feature` の振り返りで、変更がアーキテクチャや設計に影響する場合に更新します。
...
**Q: `.steering/` は add-feature のときだけ作るの？**

いいえ。原則として、AI へのあらゆる作業指示で作ります。
実装、壁打ち、レビュー、文書整理、調査のいずれでも、依頼内容と進捗が残るように `.steering/` を作成します。

...
2. `tasklist.md` は必ず以下の**4フェーズ構成**で作成すること。このプロジェクトは TDD（テスト駆動開発）を採用しており、テストインフラと統合テストを実装より先に用意するのがルールである。

   ```markdown
   ## フェーズ1: 型・テストインフラ整備（実装前に完了させる）
   - [ ] src/types/api.ts に [機能名] のエンティティ型を追加
   - [ ] src/testing/mocks/db.ts に [機能名] の DB モデルを追加
   - [ ] src/testing/mocks/handlers/[feature].ts を新規作成（MSW ハンドラー）
   - [ ] src/testing/mocks/handlers/index.ts にハンドラーを登録
   - [ ] src/testing/data-generators.ts に create[Feature]() 関数を追加

   ## フェーズ2: 統合テストを先に書く（RED）
   - [ ] src/app/routes/app/[feature]/__tests__/[feature].test.tsx を作成
         （この時点でテストが失敗する状態で OK）

   ## フェーズ3: 実装（GREEN にする）
   - [ ] src/features/[feature]/api/get-[feature]s.ts を実装
   - [ ] src/features/[feature]/api/create-[feature].ts を実装（Zod スキーマ含む）
   - [ ] src/features/[feature]/api/delete-[feature].ts を実装
   - [ ] src/features/[feature]/components/ にコンポーネントを実装
   - [ ] src/app/routes/app/[feature]/[feature]s.tsx を実装（clientLoader 含む）
   - [ ] src/config/paths.ts にパスを追加
   - [ ] src/app/router.tsx にルートを登録

   ## フェーズ4: 設定
   - [ ] .eslintrc.cjs に [feature] のクロスフィーチャーインポート禁止ルールを追加
   ```

   フェーズ1・2が完了するまでフェーズ3に進まないこと。この順序はプロジェクトの TDD ルールとして必須である。
